...
class MyDataset_PDBBind2020(Dataset):
    def __init__(self, data_path, phase, name, max_seq_len, max_smi_len, scale_target, scale_inputs, scalers):
        ...
        if scale_target:
            #self.affinity  = MinMaxScaler().fit_transform(all_df['-logKd/Ki'])  
            #self.affinity = (MinMaxScaler(feature_range=(0, 1)).fit_transform(all_df['-logKd/Ki'].reshape(-1,1))).reshape(1,-1)
...
class MyDataset_pdbbind2016(Dataset):
    ...
    def __init__(self, data_path, phase, name, max_seq_len, max_smi_len, scale_target, scale_inputs, scalers):
        ...
        #print('affinity_df.columns ', affinity_df.columns)
        
        if scale_target:  # mporw na kanw scaler ed  giati einai ola mazi training validation test
